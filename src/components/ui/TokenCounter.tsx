...
import { estimateTokenCount, getTokenCountColor, formatTokenCount, getTokenCountDescription } from '@/lib/tokenCounter';
...
export function TokenCounter({ text, className, showDetails = false, compact = false }: TokenCounterProps) {
          ...
          <TooltipContent className="bg-card/95 backdrop-blur-sm border-border/50">
            <div className="space-y-1">
              ...
              <div className="text-xs space-y-0.5">
                <p>Tokens: <span className="font-medium">{tokenInfo.tokens}</span></p>
                <p>Words: <span className="font-medium">{tokenInfo.words}</span></p>
                <p>Characters: <span className="font-medium">{tokenInfo.charactersWithSpaces}</span></p>
              </div>
            </div>
          </TooltipContent>
        </Tooltip>
      ...
      {tokenInfo.tokens > 3000 && (
        <TooltipProvider>
          <Tooltip>
            ...
            <TooltipContent className="bg-card/95 backdrop-blur-sm border-border/50">
              <p className="text-xs">Very long message - may hit token limits</p>
            </TooltipContent>
          </Tooltip>
        </TooltipProvider>
      )}
